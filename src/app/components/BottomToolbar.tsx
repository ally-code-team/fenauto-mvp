...
interface BottomToolbarProps {
  ...
  isCostsPaneExpanded: boolean;
  setIsCostsPaneExpanded: (val: boolean) => void;
...
function BottomToolbar({
  ...
  function getConnectionButtonClasses() {
    ...
    // Disconnected or connecting -> usar cor do tema ou black como fallback
    if (themeColor) {
      return `hover:opacity-90 ${cursorClass} ${baseClasses}`;
    }
    return `bg-black hover:bg-gray-900 ${cursorClass} ${baseClasses}`;
  }

  const connectionButtonStyle = !isConnected && themeColor ? 
      ...
      {showCosts && (
        <div className="flex flex-row items-center gap-2">
          <input
            id="costs"
            type="checkbox"
            checked={isCostsPaneExpanded}
            onChange={(e) => setIsCostsPaneExpanded(e.target.checked)}
            className="w-4 h-4"
          />
          <label htmlFor="costs" className="flex items-center cursor-pointer">
            {t('costs')}
          </label>
        </div>
      )}
